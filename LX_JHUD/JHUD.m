...
@property (nonatomic,strong) NSMutableArray  *customBtnArray;
...
-(void)setButtonTitleArray:(NSArray *)buttonTitleArray{
    ...
    _customBtnArray = [[NSMutableArray alloc] init];
    [buttonTitleArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        UIButton *btn = [self getBtn];
        btn.tag = idx;
        [self addSubview:btn];
        [_customBtnArray addObject:btn];
    }];
...
-(void)updateConstraints
{
    ...
    //customBtn
    __block UIButton *firstBtn = nil;
    [_customBtnArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        UIButton *btn = (UIButton *)obj;
        [btn removeAllConstraints];
        if (idx == 0) {
            [self addConstraintCenterXToView:btn centerYToView:nil];
            [self addConstarintWithTopView:self.messageLabel toBottomView:btn constarint:10];
            [btn addConstraintWidth:100 height:35];
            firstBtn = btn;
        }else{
            
            [self addConstraintCenterXToView:btn centerYToView:nil];
            [self addConstarintWithTopView:firstBtn toBottomView:btn constarint:10];
            [btn addConstraintWidth:100 height:35];
...
-(void)buttonClick:(UIButton *)btn{
    ...
    if (self.JHUDCustomButtonClickedBlock) {
        self.JHUDCustomButtonClickedBlock(btn.tag);
    }
}
